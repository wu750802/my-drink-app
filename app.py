...
# --- 4. 主畫面顯示與統計 ---
# 強制定義所有資料表的欄位結構
order_cols = ['類別', '訂單編號', '時間', '品項', '規格', '付款', '杯數', '金額', '手續費', '利潤', '狀態']
df_history = pd.DataFrame(global_data["history"], columns=order_cols)

exp_cols = ['類別', '時間', '品項', '金額', '利潤', '付款']
df_expenses = pd.DataFrame(global_data["expenses"], columns=exp_cols)
...
with col_main:
    ...
    # 修正：確保 df_expenses 即使沒資料也能正確顯示欄位
...
with col_stat:
    ...
    if not df_full_report.empty:
        ...
        total_rev = int(orders['金額'].sum())
        total_fees = round(orders['手續費'].sum(), 1)
        total_expenses = abs(int(exps['金額'].sum()))
        final_profit = int(df_full_report['利潤'].sum())
        
        m1, m2 = st.columns(2)
        m1.metric("今日總營收", f"${total_rev}")
        m1.metric("雜支總計", f"-${total_expenses}")
        m2.metric("預估淨獲利", f"${final_profit}", delta=f"平台費: -${total_fees}", delta_color="inverse")
        
        st.divider()
        if not orders.empty:
            drink_stats = orders.groupby("品項")["杯數"].sum().reindex(DRINKS, fill_value=0)
            st.bar_chart(drink_stats)
        
        if st.button("📥 下載今日總報表"):
            tw_date = get_taiwan_time().strftime('%Y%m%d')
            csv = df_full_report.to_csv(index=False).encode('utf-8-sig')
            st.download_button("確認下載總報表", csv, f"安泰穂_總報表_{tw_date}.csv", "text/csv")
